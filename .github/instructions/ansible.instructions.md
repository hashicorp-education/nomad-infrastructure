...
- Use one of the top-level deploy entrypoints from the `ansible/` directory. Each entrypoint runs common setup, installs services, bootstraps ACLs, and prints a cluster summary: `deploy_get_started.yaml` (single-node, no TLS/ACL/Consul), `deploy_consul.yaml` (Consul only), `deploy_nomad.yaml` (Nomad only), `deploy_consul_nomad_sd.yaml` (Consul + Nomad + service discovery), `deploy_consul_nomad_wi.yaml` (Consul + Nomad + workload identity), `deploy_consul_nomad_mesh.yaml` (+ service mesh), `deploy_nomad_vault.yaml` (Nomad + Vault workload identity, no Consul). Sub-playbooks in `ansible/playbooks/` can be run individually for targeted operations. There is no `site.yaml`.
