...
- [multipass-local-testing-plan.md](multipass-local-testing-plan.md) — **Proposal, not implemented.** Plan for a new `terraform/multipass/` workspace using the `larstobi/multipass` provider to stand up the same server/client topology on local Multipass VMs (for Apple Silicon Mac testing), reusing the existing Ansible roles/playbooks against a new `ansible/inventory.multipass.ini`. Found most of the Ansible layer is already cloud-agnostic (arch auto-detection in `hashicorp_release`/`cni`, Nomad's static-IP `retry_join` fallback, generic TLS SANs); the only required Ansible change is making `consul_servers.yaml`/`consul_clients.yaml`'s hardcoded AWS cloud auto-join overridable, needed only for Consul-based scenarios, not the initial Nomad-only validation target.
