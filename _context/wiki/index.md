...
- [multipass-local-testing-plan.md](multipass-local-testing-plan.md) — **Phase 0/1 implemented.** Plan and now-scaffolded `terraform/multipass/` workspace using the `larstobi/multipass` provider to stand up the same server/client topology on local Multipass VMs (for Apple Silicon Mac testing), reusing the existing Ansible roles/playbooks against a new `ansible/inventory.multipass.ini`. Most of the Ansible layer was already cloud-agnostic (arch auto-detection in `hashicorp_release`/`cni`, Nomad's static-IP `retry_join` fallback, generic TLS SANs); the one required change — making `consul_servers.yaml`/`consul_clients.yaml`/`consul_dns_token.yaml`'s hardcoded AWS cloud auto-join overridable via `consul_use_aws_cloud_join` — is applied, since the chosen first validation target is Scenario C (`deploy_consul_nomad_sd.yaml`). `terraform plan` verified clean; `terraform apply` against real VMs not yet run.
