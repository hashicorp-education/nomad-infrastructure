...
| Job template calling `{{ service "..." }}` fails with `403 (rpc error making call: ACL not found)`, even though `nomad-client-policy` and the client's Consul token both check out fine | **Confirmed bug, fixed 2026-07-20**: Play 3 of `consul_nomad_service_discovery.yaml` (writes the Nomad client's `consul {}` block) never set `nomad_client_use_consul_token: true` in its own `vars:` block. Each Ansible play's `vars:` is independent — the var set in `nomad_clients.yaml` doesn't carry over — so the role default (`false`) rendered `client { template {} }` **without** `use_client_consul_token = true`, exactly the config this doc describes at §"Nomad client agent" as the *intended* state. Consul-template then rendered with no token, hitting the anonymous (deny-all) ACL. | Check `/etc/nomad.d/nomad.hcl` for `client { template { use_client_consul_token = true } }` — if the whole `template {}` sub-block is missing entirely (not just present with `false`), this is the bug | Confirmed fixed in the current `consul_nomad_service_discovery.yaml`; if it resurfaces, re-running the playbook alone will not fix it without also restoring the `vars:` entry, since the omission reproduces identically on every run |
