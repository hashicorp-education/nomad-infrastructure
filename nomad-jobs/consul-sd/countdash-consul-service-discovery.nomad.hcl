...
job "countdash-consul-sd" {
  ...
  group "countdash-web" {
    ...
    service {
      ...
      # "${attr.unique.platform.aws.public-hostname}" as the Consul service
      # address, which then fails the health check below with something like
      # `parse "http://$%7Battr...%7D:9002/": invalid URL escape "%7B"`. If
      # you see that exact error, deployment_platform doesn't match the
