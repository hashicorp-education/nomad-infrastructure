...
This directory contains Ansible playbooks for installing and configuring a co-located HashiCorp Consul and Nomad cluster on AWS or Multipass infrastructure provisioned by Terraform. Versions are pinned in [`group_vars/all.yaml`](group_vars/all.yaml).
...
- **Use case entrypoints** (`ansible/*.yaml`): `deploy_get_started.yaml`, `deploy_consul.yaml`, `deploy_nomad.yaml`, `deploy_consul_nomad_sd.yaml`, `deploy_consul_nomad_wi.yaml`, `deploy_consul_nomad_mesh.yaml`, `deploy_nomad_vault.yaml`, `teardown.yaml`. These are the recommended entry points. Each one imports sub-playbooks in the correct order.
...
Choose one of the seven use case playbooks as the entry point. Each runs the correct sequence of individual playbooks automatically. See [DEPLOY_CLUSTER_GUIDE.md](../DEPLOY_CLUSTER_GUIDE.md) for the full per-scenario playbook breakdown.
...
flowchart TD
    ...
    DEPS --> D0["deploy_get_started.yaml"]
    ...
    DEPS --> D5["deploy_consul_nomad_mesh.yaml"]
    DEPS --> D6["deploy_nomad_vault.yaml"]

    D0 -->|"single-node, no Consul/TLS/ACL"| ENV
    D1 -->|"consul stack"| ENV
    D2 -->|"nomad stack"| ENV
    D3 -->|"consul + nomad stacks + service discovery"| ENV
    D4 -->|"consul + nomad stacks + service discovery + workload identity"| ENV
    D5 -->|"+ service mesh + API Gateway"| ENV
    D6 -->|"nomad + vault, workload identity"| ENV(["source ./set-cluster-env.sh"])
```
...
These playbooks are the recommended entry points for deployment. Each one runs from start to finish: it configures all hosts, tests Ansible connectivity, deploys the services for that scenario, and prints a cluster status summary that includes access tokens and ready-to-paste environment variable export commands.
...
| `consul_binary_version` | `2.0.2` | Consul binary version to download and install |
...
|----------|--------------------| ------------|
| `consul_binary_version` | `2.0.2` | Consul version |
...
Edit [`group_vars/all.yaml`](group_vars/all.yaml) — the single file for all
